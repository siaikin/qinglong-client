...
**不要**使用 `GET /open/crons/detail?name=…`（源码中 `/crons/detail` 仅支持 `log_path`）。应使用列表接口：

```typescript
// 模糊搜索
const { data, total } = await client.crons.list({ searchValue: '签到' });

// 按名称字段搜索
const { data } = await client.crons.list({ searchValue: 'name:每日签到' });
```

`list()` 返回 `{ data: Crontab[], total: number }`，不是裸数组。
